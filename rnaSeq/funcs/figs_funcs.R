...
plot_variable_and_stats = function(taxa, check_val, y_val, sig_ast_flag = T, print_pvals = F, y_var = 'PD_whole_tree', y_lb = 'Alpha Diversity',boxplot_p_text_flag = T, boxplot_test_type = 'wilcox', jitter_flag = T )
{
  ...
  if (is.character( taxa[[ check_val ]] ) )
  {
    ...
    if ( sig_ast_flag )
    {
      res = add_significant_asterix_to_plot_BH(p, as.factor(taxa[[check_val]]), taxa[[y_val]], print_pvals = print_pvals, test_type = boxplot_test_type)
      p=res[[1]]
      t=res[[2]]
...
format_fig = function(fig, lab = NA, title_size = 20, text_size = 16, axis_size = 12, x_angle = NA, set_ttl_flag = T)
{
  fig = fig + theme_bw() + 
...
add_significant_asterix_to_plot_BH_v2 = function(p, var, val, p_val_cutoff = 0.05, line_width = 1, print_pvals = F, log_scale_flag=F, test_type = 'perm',label_size = 8, scale_var = 6, asterix_scale_var = 1.3, manual_label = '')
{
  all_vars = levels(var)
