...
UC_file = sprintf('res/supps_v8/UC_main_validation_DE_%s_supp.txt',type)
CD_file = sprintf('res/supps_v8/CD_main_validation_DE_%s_supp.txt',type)
Celiac_file = sprintf('res/supps_v8/Celiac_main_validation_DE_%s_supp.txt',type)
...
DE_name = sprintf('../%s/res_main3TPMf_TPMf_v8/%s_%s_%s_%s_vs_%s_deseq2_res.tsv', cohort, gsub('SOURCE','SOURCE_v2',cohort), type, source, to_test[1],to_test[2])
...
table(df$Score_UC_CD_Celiac)


out_file = sprintf('res/supps_v8/main3_validated_genes_cohorts_%s_supp.txt',type)
