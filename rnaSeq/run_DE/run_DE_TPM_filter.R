...
cohort2 = gsub('SOURCE','SOURCE_v2', cohort)
rds_file = sprintf('../%s/%s_txi_%s.rds',cohort2, cohort, type)
...
{
  gene_list = read.table(sprintf('res/gene_lists/main3_TPMfilterd_%s_list_v8.txt', type))  
...
out_path = sprintf('../%s/res_v8/', cohort)
if (filter_2_main_genes & filter_by_TPM_flag)
{
  out_path = sprintf('../%s/res_main3TPMf_TPMf_v8/', cohort)
}
