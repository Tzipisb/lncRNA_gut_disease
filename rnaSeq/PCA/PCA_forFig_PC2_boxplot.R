...
make_pc2_prm_boxplot = function(c, col_prm, col_prm_name, type)
{
  ...
  metadata_df$protect_W4R = ifelse(metadata_df$protect_W4R == 0, 'noW4R','W4R')
  metadata_df$protect_W4R[metadata_df$Dx == 'Control'] = 'Control'
  metadata_df$protect_W4R = factor(metadata_df$protect_W4R, levels = c('Control','W4R','noW4R'))
  ...
  g = ggplot(data, aes(x=PC1, y=PC2)) +
    ...
    geom_point(shape = 21, aes( fill = metadata_df[[col_prm]] ), size=2) +
...
# res = make_pc2_prm_boxplot(c, col_prm, col_prm_name, type)